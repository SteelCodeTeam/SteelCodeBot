...
public class GetRankingCommand implements ISlashCommand {
    ...
    public Mono<Void> handle(ChatInputInteractionEvent event) {
        ...
        if (!optUserList.isEmpty()) {
            ...
            message = message + firstLine.formatted(optUserList.get(0).getId(), optUserList.get(0).getPoints());
            if (optUserList.size() > 1) {
                message = message + secondLine.formatted(optUserList.get(1).getId(), optUserList.get(1).getPoints());
            }
            if (optUserList.size() > 2) {
                message = message + thirdLine.formatted(optUserList.get(2).getId(), optUserList.get(2).getPoints());
            }
            if (optUserList.size() > 3) {
                message = message + fourthLine.formatted(optUserList.get(3).getId(), optUserList.get(3).getPoints());
            }
            if (optUserList.size() > 4) {
                message = message + fifthLine.formatted(optUserList.get(4).getId(), optUserList.get(4).getPoints());
            }
